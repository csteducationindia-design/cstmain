...
def calculate_fee_status(student_id):
    ...
    # If student is NOT assigned to a session, they owe 0.
    if student.session_id:
        # 1. General Session Fees (Fees with NO course assigned)
        ...
        # 2. Course Specific Fees (Only for courses the student is actually enrolled in)
        ...
        print(f"DEBUG: Student {student.name} (ID: {student.id}) has NO Session assigned. Skipping fees.")
...
def api_users():
    ...
    # Update Session
    if 'session_id' in d:
        u.session_id = int(d['session_id']) if d['session_id'] else None
...
def export(type):
    ...
    if df.empty:
        # Create empty excel with headers to prevent crash
